...
<section class="posts-grid">
    ...
    <div class="box-container">
        <?php
            $select_liked = $conn->prepare("SELECT * FROM `likes` WHERE user_id= ?"); 
            $select_liked->execute(array($user_id));
            if($select_liked->rowCount() > 0){
                while($fetch_liked = $select_liked->fetch(PDO::FETCH_ASSOC)){
                    
            $select_posts = $conn->prepare("SELECT * FROM  `posts` WHERE id= ? AND status= ? ");
            $select_posts->execute(array($fetch_liked['post_id'],'active'));
